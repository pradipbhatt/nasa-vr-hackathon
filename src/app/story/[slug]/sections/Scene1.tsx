...
export default function Scene1({ sceneRef }: SceneProps) {
  ...
  return (
    <>
      {/* ENHANCED PRELOADER */}
      <div ref={preloaderRef} className="fixed inset-0 z-[100] flex items-center justify-center bg-black transition-opacity duration-1000" style={{ display: isLoading ? "flex" : "none" }}>
        {/* Background Layers */}
        <div className="absolute inset-0 bg-gradient-to-b from-gray-950 via-slate-900 to-gray-950" />

        <div className="absolute inset-0 bg-cover bg-center opacity-40" style={{ backgroundImage: "url(/007-storm-arrived/bg-pot-02.jpg)" }} />
        ...
        <div className="relative z-[110] text-center px-8">
          <div className="mb-8 animate-pulse-slow">
            <div className="text-white text-5xl md:text-7xl font-bold mb-6 tracking-wider animate-flicker" style={{ textShadow: "0 0 30px rgba(255,255,255,0.5), 0 0 60px rgba(100,150,255,0.3)" }}>
              THE STORM AWAITS
            </div>
            <div className="text-blue-300 text-xl md:text-3xl font-light tracking-wide animate-fade-pulse" style={{ textShadow: "0 0 20px rgba(100,150,255,0.4)" }}>
          ...
          <div className="flex flex-col items-center gap-4 mt-12">
            ...
            <div className="text-blue-200 text-sm tracking-widest animate-pulse">LOADING EXPERIENCE...</div>
          </div>
        </div>
        ...
        <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 z-[120] flex gap-3">
          {[1, 2, 3].map((dot) => (
            <div key={dot} className="w-2 h-2 bg-blue-400 rounded-full animate-pulse" style={{ animationDelay: `${dot * 0.3}s` }} />
          ))}
        </div>
      </div>
      ...
      >
        ...
        <div className="min-h-screen relative flex items-center justify-center overflow-hidden bg-black">
          <div className={`absolute inset-0 transition-opacity duration-2000 ${showScene ? "opacity-100" : "opacity-0"}`}>
            {/* Background */}
            <div className="absolute inset-0 bg-cover bg-center transform scale-110" style={{ backgroundImage: "url(/001-landing/bg.png)" }} />
            ...
            <div ref={lightningRef} className="absolute inset-0 bg-gradient-to-b from-blue-100 via-white to-blue-200 opacity-0 pointer-events-none z-30 mix-blend-screen transition-opacity duration-100" />
            ...
            {/* Title */}
            <img ref={titleRef} src="/chapter-headings/chapter-1.png" alt="The Last Wave" className="absolute top-20 z-40 w-1/3 max-w-md opacity-0 animate-title left-1/2 transform -translate-x-1/2" style={{ filter: "drop-shadow(0 15px 40px rgba(0,0,0,0.9))" }} />
            ...
            {/* Scroll Indicator */}
            <div className="absolute bottom-30 left-10 z-30 opacity-0 animate-scroll-indicator">
              <img src="/001-landing/text-scroll.png" alt="Scroll to continue" className="w-32" />
            </div>
            ...
            <div className="absolute top-10 left-10 z-35 flex gap-2">
              {pages.map((_, index) => (
                <div key={index} className={`w-3 h-3 rounded-full transition-all duration-500 ${activePage === index ? "bg-blue-400 scale-125" : "bg-blue-800"}`} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="min-h-screen relative flex items-center justify-center overflow-hidden bg-black">
          <div className={`absolute inset-0 transition-opacity duration-2000 ${showScene ? "opacity-100" : "opacity-0"}`}>
            <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url(/001-landing/bg.png)" }} />
            ...
            <img ref={boatRef} src="/001-landing/boat-01.png" alt="Research Ship in Storm" className="relative z-20 w-1/2 max-w-2xl transform transition-transform duration-1000 hover:scale-105 opacity-0 animate-boat" style={{ filter: "drop-shadow(0 25px 50px rgba(0,0,0,0.95))" }} />

            <img ref={wavesRef} src="/001-landing/waves-pot.png" alt="Stormy Ocean Waves" className="absolute bottom-0 w-full opacity-0 z-15 animate-waves" style={{ filter: "contrast(1.3) brightness(0.85)" }} />
        ...
        <div className="min-h-screen relative flex items-center justify-center overflow-hidden bg-black">
          <div className={`absolute inset-0 transition-opacity duration-2000 ${showScene ? "opacity-100" : "opacity-0"}`}>
            ...
            <img src="/001-landing/cloud-pot-01.png" alt="Storm Clouds" className="absolute top-10 left-10 w-1/3 opacity-0 mix-blend-multiply z-5 animate-cloud-left" />
            <img src="/001-landing/foreground-clouds-pot.png" alt="Foreground Storm Clouds" className="absolute top-20 right-10 w-1/3 opacity-0 mix-blend-multiply z-5 animate-cloud-right" />
      ...
      <style jsx>{`
        ...
        @keyframes boat {
          0% {
            transform: translateX(-400px) translateY(80px) rotate(-8deg) scale(0.8);
            opacity: 0;
          }
          100% {
        ...
        .animate-rain-vertical-1 {
          animation: rain-vertical-1 0.8s linear infinite;
        }
        .animate-rain-vertical-2 {
          animation: rain-vertical-2 1.2s linear infinite;
        }
        .animate-rain-scene-1 {
          animation: rain-scene-1 1s linear infinite;
        }
        .animate-rain-scene-2 {
          animation: rain-scene-2 1.4s linear infinite;
        }
        .animate-flicker {
        ...
        .animate-cloud-left {
          animation: cloud-left 16s ease-in-out infinite alternate;
        }
        .animate-cloud-right {
          animation: cloud-right 20s ease-in-out infinite alternate;
        }
        .animate-waves {
          animation: waves 3.5s ease-out forwards;
        }
        .animate-boat {
          animation: boat 3s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }
        .animate-title {
          animation: title 2.5s cubic-bezier(0.68, -0.55, 0.265, 1.55) 1.5s forwards;
        }
        .animate-scroll-indicator {
          animation: scroll-indicator 2s ease-in-out 2.5s infinite;
        }
        .animate-fade-in-late {
          animation: fade-in-late 4s ease-out forwards;
        }
      `}</style>
    </>
  );
